...
class DECA(nn.Module):
    ...
    def encode(self, images, use_detail=True):
        if use_detail:
            ...
            parameters = self.E_flame(images)       # 1(batch)*236 -- {'shape': 100, 'tex': 50, 'exp': 50, 'pose': 6, 'cam': 3, 'light': 27}
    ...
    # for customized template model transformation
    # @torch.no_grad()
    # def decode(self, codedict,  iddict=None,  
    #             render_orig=False, original_image=None, tform=None):
    #     # stay 'rendering' 'use_detail' 'vis_lmk' 'return_vis' all same 
    ...
    #     # verts batch, 5023,3   landmarks2d batch, 68,3  landmarks3d batch, 68,3
